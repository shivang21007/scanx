...
# Define osquery source path based on architecture
$osquerySource = "C:\Users\Octro\Downloads\scanx\agent\dist\builds-osquery\osqueryi-5.20.0.windows_$osqueryArch.exe"
...
$copyOperations = @{
    ...
    $osquerySource = "osqueryi.exe"
    "C:\Users\Octro\Downloads\scanx\agent\config\agent.conf" = "agent.conf"
    "C:\Users\Octro\Downloads\scanx\agent\config\queries.yml" = "queries.yml"
}
...
# Link to create MSI with version
...
try {
    ...
    Write-Host "Failed to read version from agent.conf, using default: 1.6.3" -ForegroundColor Yellow
    $version = "1.6.3"
}
$msiName = "scanx-v$version.msi"
